...
*/
var BugDispatch={options:{minDelay:500,maxDelay:1E4,minBugs:2,maxBugs:20,minSpeed:5,maxSpeed:10,maxLargeTurnDeg:150,maxSmallTurnDeg:10,maxWiggleDeg:5,imageSprite:"https://auz.github.io/Bug/fly-sprite.png",bugWidth:13,bugHeight:14,num_frames:5,zoom:10,canFly:!0,canDie:!0,numDeathTypes:3,monitorMouseMovement:!1,eventDistanceToBug:40,minTimeBetweenMultipy:1E3,mouseOver:"random"},initialize:function(a){this.options=mergeOptions(this.options,a);this.options.minBugs>this.options.maxBugs&&(this.options.minBugs=this.options.maxBugs);
...
var xmax = 5;
var ymax = 3;
var fishRurl = "http://i67.tinypic.com/foq6pu.gif";
var fishLurl = "http://www.informatiquegifs.com/gifs/images/poisson/6.gif";
...
function animate() {
    ...
    if (ydir)
        if ((ty + incry + pich) > (w_y + yo)) {
            ydir = false;
            checkdirs();
            moveidby(bothfish, 0, -incry);
        } else {
            moveidby(bothfish, 0, incry);
        }
    else if ((ty - incry) < yo) {
    ...
    if (xdir)
        if ((tx + incrx + picw) > (w_x + xo)) {
            xdir = false;
            checkdirs();
            moveidby(bothfish, -incrx, 0);
        } else {
            moveidby(bothfish, incrx, 0);
        }
    else if ((tx - incrx) < xo) {
